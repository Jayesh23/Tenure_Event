...
def main_code():
	...
	while True:
		...
		if circle.size == 3:
				...
				# print("x, y, radius", x, y, radius, " type : ", type(x), type(y), type(radius))

				for client in clients:
						print("x : ", x, w, " y : ", y, h, " type : ", type(x), type(y), type(w), type(h))
						string = json.dumps({'x': float(x) / float(w), 'y': float(y) / float(h), 'radius': float(radius) / float(w)})
